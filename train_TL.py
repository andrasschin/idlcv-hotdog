...
from TL_network import ResNet50Model
...
if not os.path.exists("TL_outputs"):
    os.mkdir("TL_outputs")
...
def train(model, train_dataloader, val_dataloader, optim, loss_fn, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        with open(f'TL_outputs/results_{today}.txt', 'a') as f:
    ...
    plt.xticks(epochs[::2])
    plt.legend()
    plt.savefig(f'TL_outputs/accuracies_{today}.png')
...
if __name__ == "__main__":
    ...
    ############################ Logging ############################
    with open(f'TL_outputs/results_{today}.txt', 'w',encoding='utf-8') as f:
    ...
    try:
        ...
        test(model=model, test_dataloader=test_dataloader, loss_fn=loss_fn,device=device,save_dir='TL_outputs/saved_images')
        torch.save(model, 'ResNet50Model.pth')
